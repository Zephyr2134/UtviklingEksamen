...
function App() {
  ...
  const [aktivBruker, setAktivBruker] = useState<
    HundeEier | HundePasser | Admin | null
  >(null);
  const [nyBruker, setNyBruker] = useState<NyBruker>({
    rolle: "",
    brukernavn: "",
    passord: "",
    telefon: "",
    adresse: "",
    hund: { id: 0, navn: "", rase: "", alder: 0, spesielleBehov: "" },
    hundBildePlassering: "",
    omraade: "",
    pris: 0,
  });
  ...
  const [nyForesporsel, setNyForesporsel] = useState<Foresporsel>({
    id: 0,
    eierID: 0,
    passerID: 0,
    dato: new Date(Date.now()),
    akseptert: false,
    rapport: "",
    vurdering: 0,
    kommentar: "",
    ...
    if (lagerBruker) {
      try {
        ...
        if (plass) {
          await setNyBruker((e) => ({
            ...e,
            hundBildePlassering: `https://localhost:7130${plass}`,
          }));
  ...
  const aksepterForesporsel = async (id: number) => {
    try {
      const svar = await fetch(
        `https://localhost:7130/aksepterForesporsel/${id}`,
        {
          method: "PUT",
          headers: {
            "content-type": "application/json",
      ...
      if (svar.ok) {
        setForesporsler((gammel) =>
          gammel.map((f) => (f.id === id ? { ...f, akseptert: true } : f))
        );
  ...
  const bekreftForesporselFerdig = async (id: number) => {
    try {
      const svar = await fetch(
        `https://localhost:7130/fullforForesporsel/${id}`,
        {
          method: "PUT",
          headers: {
            "content-type": "application/json",
      ...
      if (svar.ok) {
        setForesporsler((gammel) =>
          gammel.map((f) => (f.id === id ? { ...f, fullfort: true } : f))
        );
  ...
  const foresporselBetalt = async (id: number) => {
    try {
      const svar = await fetch(
        `https://localhost:7130/foresporselBetalt/${id}`,
        {
          method: "PUT",
          headers: {
            "content-type": "application/json",
      ...
      if (svar.ok) {
        setForesporsler((gammel) =>
          gammel.map((f) => (f.id === id ? { ...f, betalt: true } : f))
        );
        console.log("Betalt");
      } else {
        console.log("Noe skjedde");
  ...
  const fullforRapport = async (id: number) => {
    try {
      const svar = await fetch(
        `https://localhost:7130/rapportForesporsel/${id}`,
        {
          method: "PUT",
          headers: {
            "content-type": "application/json",
      ...
      if (svar.ok) {
        setForesporsler((gammel) =>
          gammel.map((f) => (f.id === id ? { ...f, rapport: rapport } : f))
        );
        setRapporterer(0);
        console.log("Fullfort");
      } else {
  ...
  const fullforKommentar = async (id: number) => {
    try {
      const svar = await fetch(
        `https://localhost:7130/kommenterForesporsel/${id}`,
        {
          method: "PUT",
          headers: {
            "content-type": "application/json",
      ...
      if (svar.ok) {
        setForesporsler((gammel) =>
          gammel.map((f) =>
            f.id === id
              ? { ...f, kommentar: kommentar, vurdering: vurdering }
              : f
          )
        );
        setKommentar("");
  ...
  const deaktiverEier = async (id: number) => {
    try {
      ...
      if (svar.ok) {
        setHundeEiere((gammel) =>
          gammel.map((e) => (e.id === id ? { ...e, aktiv: false } : e))
        );
  ...
  const deaktiverPasser = async (id: number) => {
    try {
      ...
      if (svar.ok) {
        setHundePassere((gammel) =>
          gammel.map((p) => (p.id === id ? { ...p, aktiv: false } : p))
        );
        console.log("Deaktivert eier");
      } else {
        console.log("Noe skjedde");
  ...
  const aktiverEier = async (id: number) => {
    try {
      ...
      if (svar.ok) {
        setHundeEiere((gammel) =>
          gammel.map((e) => (e.id === id ? { ...e, aktiv: true } : e))
        );
    ...
    <>
      {!loggetPaa ? (
        <Login
          ...
          login={login}
          nyBruker={nyBruker}
          setNyBruker={setNyBruker}
          haandterBildeEndring={haandterBildeEndring}
          bilde={bilde}
        />
      ) : (
        <>
          {aktivBruker && aktivBruker.brukernavn === "admin" ? (
            <div>
              <button className="Hundeknapp loggUt" onClick={() => loggUt()}>
                🐾 Logg ut
              </button>
              <Admin
                eiere={hundeEiere}
                passere={hundePassere}
                hunder={hunder}
                foresporsler={foresporsler}
                aktiverEier={aktiverEier}
                aktiverPasser={aktiverPasser}
                deaktiverEier={deaktiverEier}
                deaktiverPasser={deaktiverPasser}
              />
            </div>
            ...
            <div
              className="EierSide"
              style={{
                backgroundImage: `url(${aktivBruker.hundBildePlassering})`,
              }}
            >
              <button className="Hundeknapp loggUt" onClick={() => loggUt()}>
                🐾 Logg ut
              ...
              <Foresporsler
                ...
                eiere={hundeEiere}
                passere={hundePassere}
                lagForesporsel={lagForesporsel}
                redigererForesporsel={redigererForesporsel}
                fullforForesporsel={fullforForesporsel}
                nyForesporsel={nyForesporsel}
                setNyForesporsel={setNyForesporsel}
                aksepterForesporsel={aksepterForesporsel}
                valgtOmraade={valgtOmraade}
                setValgtOmraade={setValgtOmraade}
                hunder={hunder}
                bekreftForesporselFerdig={bekreftForesporselFerdig}
                rapport={rapport}
                setRapport={setRapport}
                fullforRapport={fullforRapport}
                rapporterer={rapporterer}
                setRapporterer={setRapporterer}
                kommenterer={kommenterer}
                setKommenterer={setKommenterer}
                fullforKommentar={fullforKommentar}
                setVurdering={setVurdering}
                setKommentar={setKommentar}
                foresporselBetalt={foresporselBetalt}
                valgtPris={valgtPris}
                setValgtPris={setValgtPris}
              />
            </div>
          ) : (
            <div>
